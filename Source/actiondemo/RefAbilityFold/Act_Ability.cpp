...
void UAct_Ability::PreActivate(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo,const FGameplayAbilityActivationInfo ActivationInfo,FOnGameplayAbilityEnded::FDelegate* OnGameplayAbilityEndedDelegate, const FGameplayEventData* TriggerEventData)
{
	...
	OnRealesedDelegate.AddUObject(this,&UAct_Ability::OnHoldEnded);
	//技能前摇以及单个技能任务的创建
	...
	{	//当有任务的时候就播放 TODO::需要处理所有动画在受伤时被打断后的输入。（额不对貌似不用只用在手上这里也添加即可）
		ContinueTagTask=UAbilityAsync_WaitGameplayEvent::WaitGameplayEventToActor(GetAvatarActorFromActorInfo(),ActTagContainer::ExeMulityInputRelaxAttack,false,true);
...
{	
	int32 level=FMath::Clamp(int32(AbilityTriggerTime/HoldLevelTime),1,HoldPostMontage.Num());
	return level;
}
void UAct_Ability::OnHoldEnded()
{
	//当松开时，播放post内容结束后摇随后结束技能
	if (UAnimMontage* Montage=HoldPostMontage[CaculateAbilityHoldLevel(HoldUpLevelTime)])
	{
