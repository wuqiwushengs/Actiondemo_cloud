#pragma once
#include "CoreMinimal.h"
#include "NativeGameplayTags.h"

namespace ActTagContainer
{	//Native
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(CharacterMoveAround);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(CharacterCameraMoveAround);
	//Ability Input
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(AbilityTest);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(InputRelaxAttack);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(InputHeavyAttack);
	//锁定因为牵涉到摄像机而非人物的技能，因此实际上是在native部分进行绑定
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(LockTarget);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(InputDefense);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(InputRolling);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(InputRestController)
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(InputDashDirection)
	//Ability 这里是每个招式的AbilityTag;
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(RelaxAttack);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(HeavyAttack);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Defense);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(DefenseRelaxAttack);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(DefenseHeavyAttack);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Rolling);
	//输入中是否按下
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Pressed);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Released);
	//预输入执行Tag
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ExePreInputRelaxAttack);
	//单一技能连击输入Tag
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ExeMulityInputRelaxAttack);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ExeMulityInputExeAttack);
	//打断角色技能的Tag
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ExeInterrupt);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(InterruptHurt);
	//松开技能的Tag
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ExeHoldAbilityInputRelaxAttackReleased);
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ExeUnComboAbilityInputReleased);
	//角色状态的tag
	//无敌状态
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(UnBeatable)
	//虚弱状态
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(WeakState)
	//从虚弱到正常的tag
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Weak2Normal)
	//控制角色的Attribute数据
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(DamageValue)
	//攻击中的效果
	ACTIONDEMO_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(AttackTimeDilamation)
}

